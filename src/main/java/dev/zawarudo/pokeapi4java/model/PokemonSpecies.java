...
public class PokemonSpecies implements Comparable<PokemonSpecies> {
	...
	public static class PokedexEntry {
		...
		public String getCleanText(String name) {
			return text.replace("\n", " ")
					...
					.replace(name.toUpperCase(Locale.UK), Formatter.format(name));
		}
	}
	...
	public String getEvolutionChainString() {
		...
		if (evolution.chain.evolvesTo.isEmpty()) {
			sb.append(Formatter.format(name));
		}

		// One evolution
		else if (evolution.chain.evolvesTo.get(0).evolvesTo.isEmpty()) {
			String stage1 = Formatter.format(evolution.chain.species.getName());
			sb.append(stage1).append(" → ").append(Formatter.format(evolution.chain.evolvesTo.get(0).species.getName()));

			for (int i = 1; i < evolution.chain.evolvesTo.size(); i++) {
				sb.append("\n").append(stage1).append(" → ").append(Formatter.format(evolution.chain.evolvesTo.get(i).species.getName()));
			}
		}
		...
		// Two evolutions
			...
			if (evolution.chain.evolvesTo.size() > 1) {
				sb.append(stage1).append(" → ").append(Formatter.format(evolution.chain.evolvesTo.get(0).species.getName()))
								 .append(" → ").append(Formatter.format(evolution.chain.evolvesTo.get(0).evolvesTo.get(0).species.getName()));

				for (int i = 1; i < evolution.chain.evolvesTo.size(); i++) {
					sb.append("\n").append(stage1).append(" → ").append(Formatter.format(evolution.chain.evolvesTo.get(i).species.getName()))
												  .append(" → ").append(Formatter.format(evolution.chain.evolvesTo.get(i).evolvesTo.get(0).species.getName()));
				}
			}
			// Only one stage 2 evolution
			else {
				String stage2 = Formatter.format(evolution.chain.evolvesTo.get(0).species.getName());
				sb.append(stage1).append(" → ").append(stage2).append(" → ").append(Formatter.format(evolution.chain.evolvesTo.get(0).evolvesTo.get(0).species.getName()));

				for (int i = 1; i < evolution.chain.evolvesTo.get(0).evolvesTo.size(); i++) {
					sb.append("\n").append(stage1).append(" → ").append(stage2).append(" → ").append(Formatter.format(evolution.chain.evolvesTo.get(0).evolvesTo.get(i).species.getName()));
				}
			}
		}
		return sb.toString();
	 ...
	 */
	public Pokemon getPokemon() throws IOException, InvalidPokedexIdException {
